...
public class ExcelExporter {
    public static void exportToExcel(String date, List<CovidStatus> covidStatusList, String fileName) throws IOException {
        ...
        for(int i = 0; i < covidStatusList.size(); i++){
            ...
            row.createCell(4).setCellValue(covidStatus.getRegion());
            row.createCell(5).setCellValue(covidStatus.getRegion());
            row.createCell(6).setCellValue(covidStatus.getRegion());
        }
